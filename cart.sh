...
curl -o $Temp_Folder  https://roboshop-artifacts.s3.amazonaws.com/cart-v3.zip &>> $LOG_FILE
...
END_TIME_LOG=$(date +%Y-%m-%d_%H-%M-%S)

Total_time=$(( END_TIME_STAMP - START_TIME_STAMP ))

echo "script excuted in $Total_time seconds"

echo -e "\njabil application $component component completed at $END_TIME_LOG"
